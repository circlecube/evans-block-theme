...
Core blocks are "built-in" via the (Gutenberg) block editor. These are the familiar blocks like paragraph, image, quote and heading as well as some layout blocks like cover blocks and columns. These are very stable but are still being improved with every release.
